...
app.get("/download/:collectionName", async (req, res) => {
  ...
  try {
    const Contact = mongoose.model(collectionName, collectionSchema);
    const data = await Contact.find({}, { _id: 0, __v: 0 }); // Exclude _id and __v fields

    const content = data.map(item => item.email).join("\n");

    res.setHeader("Content-Disposition", `attachment; filename=${collectionName}.txt`);
